...
import { exchangeOAuthCode } from "@/lib/social/tiktok";
...
export async function GET(req: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(new URL("/dashboard?error=tt_oauth_missing", env.appUrl));
  }
  if (state !== cookieState) {
    return NextResponse.redirect(new URL("/dashboard?error=tt_oauth_state", env.appUrl));
  }
  ...
  try {
    ...
    const res = NextResponse.redirect(new URL(`/clients/${clientId}/social?connected=tiktok`, env.appUrl));
